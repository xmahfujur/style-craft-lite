...
export default function CheckoutForm() {
  ...
  const isFreeShipping = (Array.from(selectedProducts.values()) as number[]).reduce((a, b) => a + b, 0) >= 2;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name || !form.address || !form.phone || selectedProducts.size === 0 || !size) {
      setShowErrors(true);
      setNotification('দয়া করে সবগুলো ঘর পূরণ করুন এবং সাইজ সিলেক্ট করুন');
  ...
  if (isSuccess) {
    return (
      <motion.div 
        ...
        className="max-w-xl mx-auto py-20 px-4 text-center"
      >
        ...
        <button 
          ...
          className="px-8 py-3 bg-green-600 text-white rounded-xl font-bold"
        >
          আরও শার্ট দেখুন
        </button>
      </motion.div>
    ...
    <section id="checkout-form" className="py-20 px-4 max-w-5xl mx-auto relative">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
          <div className={`bg-white p-6 rounded-2xl border-2 transition-all ${showErrors && selectedProducts.size === 0 ? 'border-rose-500 shadow-rose-100 shadow-xl' : 'border-gray-100 shadow-xl shadow-gray-100'}`}>
          ...
          <div className={`bg-white p-6 rounded-2xl border-2 transition-all ${showErrors && !size ? 'border-rose-500 shadow-rose-100 shadow-xl' : 'border-gray-100 shadow-xl shadow-gray-100'}`}>
            <h3 className="text-xl font-bold mb-6 flex items-center justify-between">
              আপনার বডি সাইজ কি?
              {!size && <span className={`${showErrors ? 'text-rose-600' : 'text-rose-500'} text-xs font-black animate-pulse uppercase tracking-widest`}>সিলেক্ট করুন *</span>}
            </h3>
                ...
                <button
                  ...
                  className={`w-14 h-14 rounded-xl border-2 font-bold transition-all ${
                    size === s 
                      ...
                      : (showErrors && !size) ? 'border-rose-200 bg-rose-50/30 text-rose-400' : 'border-gray-100 hover:border-gray-200 text-gray-600'
                  }`}
                >
                  {s}
                </button>
                  ...
                  return (
                    <div key={id} className="flex justify-between items-center text-sm">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 flex items-center justify-center bg-gray-100 rounded text-[10px] font-bold">{qty}x</span>
                        <span className="text-gray-600 truncate max-w-[150px]">{product.name}</span>
                      </div>
                      <span className="font-bold text-gray-900">৳{product.price * qty}</span>
                    </div>
                  );
                })}
              </div>
