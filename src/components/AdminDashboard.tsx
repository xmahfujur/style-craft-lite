...
import { Package, Phone, MapPin, Calendar, Trash2, Printer, LogOut, LayoutDashboard, ShoppingCart, Users, User as UserIcon, Settings, Search, ChevronLeft, ChevronRight, CheckCircle, XCircle, Table as TableIcon, Trello, PlusCircle, Plus, Minus, Edit2, Save, Download, Filter } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const deleteOrder = async (orderId: string) => {
    if (window.confirm('Are you sure you want to delete this order document permanently?')) {
      try {
        await deleteDoc(doc(db, 'orders', orderId));
        setNotification({ message: 'Order deleted successfully!', type: 'success' });
      } catch (error) {
        setNotification({ message: 'Permission denied. You may not have authority to delete orders.', type: 'error' });
        try {
  ...
  const clearCancelledOrders = async () => {
    ...
    if (cancelled.length === 0) {
      setNotification({ message: 'No cancelled orders to clear.', type: 'success' });
      return;
    }

    if (window.confirm(`Are you sure you want to permanently delete all ${cancelled.length} cancelled orders?`)) {
      setLoading(true);
      try {
        ...
        await batch.commit();
        setNotification({ message: `Successfully cleared ${cancelled.length} cancelled orders.`, type: 'success' });
      } catch (error) {
        setNotification({ message: 'Failed to clear orders. Some items might remain.', type: 'error' });
        try { handleFirestoreError(error, OperationType.WRITE, 'orders/batch'); } catch(e){}
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleBulkUpdate = async () => {
    ...
    try {
      ...
      selectedOrderIds.forEach(id => {
        batch.update(doc(db, 'orders', id), { status: bulkStatus });
      });
      await batch.commit();
      setNotification({ message: `Updated ${selectedOrderIds.length} orders to ${bulkStatus}`, type: 'success' });
      setSelectedOrderIds([]);
      setBulkStatus('');
    } catch (error) {
      setNotification({ message: 'Bulk update failed', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    const rows = orders.map(o => {
      ...
      return `"${o.id}","${o.customerName}","${o.phoneNumber}","${o.address.replace(/"/g, '""')}","${o.status}",${o.totalPrice},"${date}"`;
    });
  ...
  const salesData = (() => {
    ...
    orders.filter(o => o.status !== 'cancelled').forEach(o => {
      const day = o.createdAt?.toDate ? o.createdAt.toDate().toLocaleDateString() : null;
      if (day && dataMap.has(day)) {
        dataMap.set(day, dataMap.get(day) + o.totalPrice);
      }
    });

    return last7Days.map(day => ({
      name: day.split('/')[0] + '/' + day.split('/')[1],
      total: dataMap.get(day)
    }));
  })();
  ...
  const customerStats = (() => {
    ...
    orders.forEach(o => {
      ...
      if (!stats.has(key)) {
        stats.set(key, { name: o.customerName, totalSpend: 0, orderCount: 0, lastOrder: o.createdAt });
      }
      const s = stats.get(key);
      s.totalSpend += o.totalPrice;
      s.orderCount += 1;
      if (o.createdAt?.seconds > (s.lastOrder?.seconds || 0)) {
        s.lastOrder = o.createdAt;
      }
    });
    return Array.from(stats.entries()).map(([phone, data]) => ({ phone, ...data }))
          ...
          <div className="w-[210mm] h-[148.5mm] overflow-hidden bg-white text-black font-sans box-border px-8 py-6 relative flex flex-col mx-auto">
            ...
            <div className="grid grid-cols-2 gap-8 mb-4 shrink-0">
              ...
              <div className="text-right p-3">
                ...
                <p className="text-[10px] text-slate-500">Mirpur, Dhaka, Bangladesh</p>
                <p className="text-[10px] text-slate-500">Tel: +880 14100 40330</p>
              </div>
            </div>
            ...
            <div className="flex justify-between items-end mt-4 pt-4 border-t-2 border-slate-800 shrink-0">
              <div className="text-[9px] text-slate-400 font-medium max-w-[200px]">
                Goods remain the property of StyleCraft until paid in full. Returns accepted within 3 days.
              </div>
        ...
        <main className="flex-1 overflow-x-auto overflow-y-hidden p-4 md:p-8 relative">
          ...
          {loading ? (
            ...
            <>
              ...
              {activeTab === 'dashboard' && (
                <div className="h-full overflow-y-auto pr-2 pb-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center group relative">
                      ...
                      <div className="flex justify-between items-end mt-2">
                        <p className="text-xs text-red-500 font-bold flex items-center gap-1">Need attention</p>
                        {cancelledOrders > 0 && (
                          <button 
                            onClick={clearCancelledOrders}
                            className="text-[10px] bg-red-50 text-red-600 px-2 py-1 rounded-md font-bold hover:bg-red-600 hover:text-white transition-all opacity-0 group-hover:opacity-100"
                          >
                            Clear All
                          </button>
                        )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-6 min-h-[350px] flex flex-col relative overflow-hidden">
                      <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold text-slate-900">Revenue Analysis (Last 7 Days)</h3>
                        <div className="flex items-center gap-2 text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md font-black uppercase tracking-widest">
                          Real-time
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div className="flex items-center gap-3">
                      <select 
                        ...
                        className="bg-white border border-slate-200 text-slate-700 text-xs rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500/20"
                      >
                      ...
                      <button 
                        ...
                        className="bg-slate-900 text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-slate-800 disabled:bg-slate-300 disabled:cursor-not-allowed transition-all"
                      >
                        Apply to {selectedOrderIds.length} Selected
                      </button>
                    </div>
                    <button 
                      onClick={exportToCSV}
                      className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 text-xs font-bold px-4 py-2 rounded-lg hover:bg-slate-50 transition-all shadow-sm"
                    >
                      <Download size={14} />
                      Export Orders CSV
                    </button>
                      ...
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {filteredOrders.length === 0 ? (
                             <tr>
                               <td colSpan={7} className="px-6 py-12 text-center text-slate-500 text-sm">No orders found matching criteria.</td>
                             </tr>
                          ) : (
                            filteredOrders.map(order => {
                              const stageColor = PIPELINE_STAGES.find(s => s.id === order.status)?.color || 'bg-slate-100 text-slate-800';
                              return (
                              <tr key={order.id} className={`hover:bg-slate-50/50 transition-colors group ${selectedOrderIds.includes(order.id) ? 'bg-indigo-50/30' : ''}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Stock Count</label>
                                <input 
                                  type="number" 
                                  className="w-full border border-slate-200 rounded-lg p-3 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" 
              ...
              {/* INVENTORY VIEW */}
              {activeTab === 'inventory' && (
                <div className="h-full overflow-y-auto pr-2 pb-8 custom-scrollbar">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                       <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Total SKU</p>
                       <h3 className="text-2xl font-black text-slate-900">{products.length}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                       <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Low Stock</p>
                       <h3 className="text-2xl font-black text-amber-600 font-mono">{products.filter(p => (p.stock || 0) > 0 && (p.stock || 0) < 10).length}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                       <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Out of Stock</p>
                       <h3 className="text-2xl font-black text-rose-600 font-mono">{products.filter(p => (p.stock || 0) <= 0).length}</h3>
                    </div>
                  </div>
                        ...
                        {products.map(product => {
                          ...
                          return (
                            <tr key={product.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-6 py-4 font-mono font-bold text-slate-700 text-center">{stock}</td>
                              <td className="px-6 py-4">
                                {stock > 10 ? (
                                  <span className="bg-green-100 text-green-700 px-2 py-1 rounded text-[10px] font-black uppercase">Good</span>
                                ) : stock > 0 ? (
                                  <span className="bg-amber-100 text-amber-700 px-2 py-1 rounded text-[10px] font-black uppercase">Low</span>
                                ) : (
                                  <span className="bg-rose-100 text-rose-700 px-2 py-1 rounded text-[10px] font-black uppercase">Alert</span>
                                )}
                              </td>
              ...
              {activeTab === 'customers' && (
                <div className="h-full overflow-y-auto pr-2 pb-8 custom-scrollbar">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center">
                      <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-1">Total Customers</p>
                      <h3 className="text-3xl font-black text-slate-900">{customerStats.length}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center">
                      <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-1">Avg. LTV</p>
                      <h3 className="text-3xl font-black text-indigo-600">
                        ৳{(customerStats.reduce((acc, c) => acc + c.totalSpend, 0) / (customerStats.length || 1)).toLocaleString()}
                      </h3>
                    </div>
                  </div>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-slate-500 flex items-center gap-1"><Phone size={10} /> {customer.phone}</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                               <span className="font-mono font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded-md text-xs">{customer.orderCount} Orders</span>
                            </td>
                            <td className="px-6 py-4">
                               <p className="text-sm font-black text-slate-900">৳{customer.totalSpend.toLocaleString()}</p>
                            </td>
                            <td className="px-6 py-4">
                               <p className="text-xs text-slate-500 font-medium">
                                 {customer.lastOrder?.toDate ? customer.lastOrder.toDate().toLocaleDateString() : 'N/A'}
                               </p>
                            </td>
                          </tr>
                        ))}
